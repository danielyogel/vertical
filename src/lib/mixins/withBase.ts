...
export type BaseParams<V, S, VM extends Pick<Node<V, S>, 'value' | 'store' | 'index' | 'children'>> = MetaParams & {
  ...
  isSelected?: isSelected<V, S, VM & Pick<Node<V, S>, 'id' | 'label' | 'placeholder' | 'isLoading' | 'setLoading' | 'errors' | 'isDisabled' | 'autoFocus'>>;
  isVisible?: isVisible<V, S, VM & Pick<Node<V, S>, 'id' | 'label' | 'placeholder' | 'isLoading' | 'setLoading' | 'errors' | 'isDisabled' | 'isSelected' | 'autoFocus'>>;
};

export default function withBase<V, S, VM extends Pick<Node<V, S>, 'value' | 'store' | 'onStoreChange' | 'index' | 'children'>>(params?: BaseParams<V, S, VM>) {
